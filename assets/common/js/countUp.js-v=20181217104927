        ...
        return $(this).each(function() {
            ...
            function updateTimer() {
                ...
                if(this.sizeNumBefore>=options.beforeSize)  {
               		...
               		$(_this).html(options.firstSymbol+str+options.lastSymbol);

                } else{
                    //在<的时候 前面要补0 再显示
                    this._str = Array(options.beforeSize-this.sizeNumBefore + 1).join('0') + str;
                	$(_this).html(options.firstSymbol+this._str+options.lastSymbol);
                }          
                ...
                if (loopCount >= loops) {//over
                    ...
                    $(_this).html(options.firstSymbol+originalData+options.lastSymbol);
                    value = $(_this).text();
